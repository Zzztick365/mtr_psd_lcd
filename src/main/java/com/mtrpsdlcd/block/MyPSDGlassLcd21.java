...
public class MyPSDGlassLcd21 extends BlockPSDGlass {
	...
	public ActionResult onUse2(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult blockHitResult) {
		return IBlock.checkHoldingBrush(world, player, () -> {
			...
			for (int dy = -1; dy <= 1; dy++) {
				...
				if (upState.getBlock().data instanceof MyPSDGlassLcd21) {
			...
			if (topState.getBlock().data instanceof MyPSDTopLcd21) {
	...
	private void toggleTopPersistent(World world, BlockPos topPos, BlockPSDTop.EnumPersistent value) {
		...
		if (top.getBlock().data instanceof MyPSDTopLcd21) {
	...
	private void propagateGlassChain(World world, BlockPos glassPos, Direction dir, BlockPSDTop.EnumPersistent value) {
		for (int i = 1; ; i++) {
			...
			if (!(nextState.getBlock().data instanceof MyPSDGlassLcd21)) {
				break;
			}
	...
	private void connectGlass(World world, BlockPos pos, BlockState state) {
		...
		final boolean hasLeft = leftState.getBlock().data instanceof MyPSDGlassLcd21;
		if (hasLeft) {
		...
		final boolean hasRight = rightState.getBlock().data instanceof MyPSDGlassLcd21;
		if (hasRight) {
	...
	private void recalcGlassSide(World world, BlockPos pos, Direction facing, Direction dir) {
		for (int i = 1; ; i++) {
			...
			final boolean isCol = neighbor.getBlock().data instanceof MyPSDGlassLcd21;
			if (!isCol) {
				break;
			}
			...
			if (world.getBlockState(other).getBlock().data instanceof MyPSDGlassLcd21) {
	...
	private void recalcOneGlassTop(World world, BlockPos pos, Direction facing) {
		...
		final boolean hasLeft = world.getBlockState(pos.offset(facing.rotateYCounterclockwise())).getBlock().data instanceof MyPSDGlassLcd21;
		final boolean hasRight = world.getBlockState(pos.offset(facing.rotateYClockwise())).getBlock().data instanceof MyPSDGlassLcd21;
		final IBlock.EnumSide ownSide = hasLeft && hasRight ? IBlock.EnumSide.MIDDLE : (hasLeft ? IBlock.EnumSide.RIGHT : (hasRight ? IBlock.EnumSide.LEFT : IBlock.EnumSide.SINGLE));
		world.setBlockState(pos, state.with(new Property<>(IBlock.SIDE_EXTENDED.data), ownSide));
	}
}
